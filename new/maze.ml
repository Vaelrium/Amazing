...
module	Maze : MAZE =
  struct
    ...
    let check_joined one two pos =
      ...
      else if pos == 2 && Gate.Gate.down d1 && Gate.Gate.up d2 then true
      else if pos == 3 && Gate.Gate.up d1 && Gate.Gate.down d2 then true
      else false
    ...
    let rec uniformiz_idz (x, cases) to_change id =
      ...
      if (vx + 1) < x then (if (check_diff to_change cases.(vy).(vx+1)) && (check_joined to_change cases.(vy).(vx+1) 0) then uniformiz_idz (x, cases) cases.(vy).(vx+1) id);
      ...
      if (vy + 1) < x then (if (check_diff to_change cases.(vy+1).(vx)) && (check_joined to_change cases.(vy+1).(vx) 2) then uniformiz_idz (x, cases) cases.(vy+1).(vx) id);
    ...
    let rec find_random (x, cases) =
      ...
      let first = cases.(vy).(vx) in
      if dir == 0 && (vx + 1) < x && check_diff first cases.(vy).(vx + 1) then (first, cases.(vy).(vx + 1), 0)
      else if dir == 1 && (vx - 1) >= 0 && check_diff first cases.(vy).(vx - 1) then (first, cases.(vy).(vx - 1), 1)
      else if dir == 2 && (vy + 1) < x && check_diff first cases.(vy + 1).(vx) then (first, cases.(vy + 1).(vx), 2)
      else if dir == 3 && (vy - 1) >= 0 && check_diff first cases.(vy - 1).(vx) then (first, cases.(vy - 1).(vx), 3)
    ...
    let one_join (x, cases) =
      ...
      else if dir == 2 then (Gate.Gate.sd doors1; Gate.Gate.su doors2)
      else (Gate.Gate.su doors1; Gate.Gate.sd doors2);
