...
module	Maze : MAZE =
  struct
    ...
    let rec find_random (x, cases) =
      ...
      if (vx + 1) < x && Case.Case.get_id first != Case.Case.get_id cases.(vx + 1).(vy) then (first, cases.(vx + 1).(vy), 0)
      else if (vx - 1) >= 0 && Case.Case.get_id first != Case.Case.get_id cases.(vx - 1).(vy) then (first, cases.(vx - 1).(vy), 1)
      else if (vy + 1) < x && Case.Case.get_id first != Case.Case.get_id cases.(vx).(vy + 1) then (first, cases.(vx).(vy + x), 2)
      else if (vy - 1) >= 0 && Case.Case.get_id first != Case.Case.get_id cases.(vx).(vy - 1) then (first, cases.(vx).(vy - x), 3)
    ...
    let rec uniformiz_idz (x, cases) to_change id =
      ...
      if (vx + 1) < x && (check_diff to_change cases.(vx+1).(vy)) && (check_joined to_change cases.(vx+1).(vy) 0) then uniformiz_idz (x, cases) cases.(vx+1).(vy) id;
      if (vx - 1) >= 0 && (check_diff to_change cases.(vx-1).(vy)) && (check_joined to_change cases.(vx-1).(vy) 1) then uniformiz_idz (x, cases) cases.(vx-1).(vy) id;
      if (vy + 1) < x && (check_diff to_change cases.(vx).(vy+1)) && (check_joined to_change cases.(vx).(vy+1) 2) then uniformiz_idz (x, cases) cases.(vx).(vy+1) id;
      if (vy - 1) >= 0 && (check_diff to_change cases.(vx).(vy-1)) && (check_joined to_change cases.(vx).(vy-1) 3) then uniformiz_idz (x, cases) cases.(vx).(vy-1) id

    let one_join (x, cases) =
      let (first, second, dir) = find_random (x, cases) in
      print_endline "lol";
      Printf.printf "x1 %d y1 %d x2 %d y2 %d\n" (Case.Case.get_x first) (Case.Case.get_y first) (Case.Case.get_x second) (Case.Case.get_y second); (* born to test *)
    ...
    let rec make_perfect (x, maze) ctr = (* delete ctr *)
      if check_perfect (x, maze) || ctr = 10 then ()
      else one_join (x, maze);
      make_perfect (x, maze) (ctr+1)
    ...
    let joined_right (x, maze) vy vx =
      ...
      if vx == x then false
      else if Gate.Gate.right doors1 && Gate.Gate.left doors2 then true
      else false

    let rec aff_line (x, maze) y ctr =
      if ctr = 0 then Printf.printf "|";
      if y != (x -1) then (if (joined_down (x, maze) y ctr) = false then Printf.printf "_" else Printf.printf " ")
      else Printf.printf "_";
      if ctr != (x - 1) then (if (joined_right (x, maze) y ctr) = false then Printf.printf "|" else Printf.printf ".")
      else (print_endline "|");
